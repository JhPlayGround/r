# Stacked autoencoder
# 한층씩 순차적으로 훈련
install.packages ("C:/SAENET_1.1.tar.gz", repos=NULL, type="source")
library(SAENET)
...
fit = SAENET.train(X.train=data1[train, ], n.nodes=c(5,4,2), unit.type="logistic",
...
fit[[3]]$X.output
plot(fit[[3]]$X.output, xlab="node 1", ylab="node 2")
title("Features from 3rd layer")
